...
### 论文规模 *随机初始化* 的真实数据结果（chain3，1500 step×3 阶段）

| clip | 尺度 | 未训练 rel | 训练后 rel | 未训练 comp | 训练后 comp |
|---|---|---|---|---|---|
| office | 82 m | 0.0243 | 0.0244 | 1.203 m | 1.177 m |
| pipes | 48 m | 0.0399 | 0.0400 | 0.553 m | 0.644 m |
| statue | 45 m | 0.0484 | 0.0455 | 0.606 m | 0.672 m |
| courtyard | 165 m | 0.0668 | **0.0576** | 2.538 m | **2.164 m** |
| colmap（稠密小物体） | 8.6 m | 0.3325 | 0.3255 | — | 0.169 m |

结论要谨慎陈述：**5-clip 均值 0.0986 与前面 4-clip 的 0.0449 不可直接比**（多了一个小尺度稠密 clip，
它自身 rel≈0.33 就把均值抬高）。逐 clip 看，随机初始化的 1.16B 模型在 1500 step 内几乎不动
（office/pipes 持平，courtyard −14%、statue −6%），而 768d + DINOv2 初始化能到 0.0396（-11% 且内点率大幅提升）。
这正指向论文的前提：**没有预训练 4D 层级，光靠 1K 级监督很难把大模型训起来**。
下一步（正在跑）：论文规模 + **真实 4RC 初始化**在同一 5-clip 池上的对照。

### 记录：chain4 的评测步作废（守卫按预期拦住）

chain4（4RC backbone + 旧 `mlp` 相机头）训练有效；但我在它运行期间把默认布局切到 `cam_dec`，
它的检查点里 `decoder.camera_head.mlp.*` 在新模型里已不存在。`load_checkpoint` 的
dropped-tensors 报告把这件事**显式打了出来**，所以那一轮 eval 数字不采信 —— 这正是该守卫存在的理由
（没有它，我们会安静地把随机相机头当成训练结果来汇报）。真正采信的运行是 chain5（backbone + 预训练相机头）。

### Table 3 现在必须逐变体训练

`tools/eval_gt.py` 之前只能吃合成 clip，且不训练任何变体 —— 输出的表看着能比、其实全是随机头
（实测五个变体 Acc 全在 3.1~3.5 之间，毫无判别力）。现在它接受 `--data <pool.yaml>`，
`scripts/run_table3.sh` 对每个变体生成配置、各自训练后再评，并只把论文 7-Scenes/NRGBD 行作为**参照**打印。
